...
| 환경 | 실행 주체 | 정의 위치 |
| --- | --- | --- |
| **온프레미스** (로컬 전체 컨테이너) | 개발자 PC / 온프레미스 호스트 | [`infra/onprem/`](../../infra/onprem/) |
| **AWS 클라우드** (앱·DB·모니터링 EC2 3대) | GitHub Actions → ECR → EC2 pull | [`infra/cloud/`](../../infra/README.md) · 절차 [cloud-deploy.md](cloud-deploy.md) |
...
docker compose --env-file .env --profile observability --profile edge up -d  # +외부 임시 URL
```

→ http://localhost (nginx 현관). 종료: `docker compose --profile observability --profile edge down` (데이터 볼륨 유지).
...
설정 파일: [`infra/onprem/monitoring/`](../../infra/onprem/monitoring/) (`prometheus.yml` · `loki-config.yml` · `promtail-config.yml`).

## 3. 자주 하는 운영 작업

> 온프레미스 컨테이너 작업은 [`infra/onprem/`](../../infra/onprem/)에서 실행한다(`cd infra/onprem`).
...
| 프론트만 안 뜸 | next 컨테이너 로그, nginx 프록시 설정(`infra/onprem/nginx.conf`) 확인 |
