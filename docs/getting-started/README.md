...
| `GRAFANA_ADMIN_USER` · `GRAFANA_ADMIN_PASSWORD` | Grafana 로그인 | 기본값 있음(observability 프로파일에서만) |
...
### B. local-deploy — 전부 Docker (운영 패리티·시연)
...
> 관측(Grafana 등)·외부노출은 프로파일을 추가한다. 상세·트러블슈팅은 루트 [README.md](../../README.md).
...
- local-deploy 모드에서는 위 주소의 포트 대신 **http://localhost** 및 `http://localhost/api/...` 로 접근한다.
...
| 종료 | `docker compose --profile web down` (데이터 볼륨은 유지) |
