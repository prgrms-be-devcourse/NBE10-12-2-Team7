...
## 현재 검증 상태
- [x] `GET /api/members/me` 5가지 케이스 확인 완료 (성공/토큰없음/유효하지않은토큰/DELETED회원/SUSPENDED회원)
- [x] `PATCH /api/members/me` 6가지 케이스 확인 완료 (성공/토큰없음/닉네임중복/validation/DELETED회원/SUSPENDED회원)
- [x] `DELETE /api/members/me` 5가지 케이스 확인 완료 (성공/탈퇴후로그인/토큰없음/재탈퇴시도/SUSPENDED회원)
...
JWT가 유효하더라도 탈퇴 처리된 회원은 접근이 차단된다.
...
```json
{
  ...
  "message": "탈퇴한 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.0216008"
}
```
...
```json
{
  ...
  "message": "정지된 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.3988059"
}
```
...
```json
{
  ...
  "error": "DELETED_MEMBER",
  "message": "탈퇴한 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.1494408"
}
```
...
```json
{
  ...
  "error": "SUSPENDED_MEMBER",
  "message": "정지된 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.5222144"
}
```
...
- 이미 탈퇴한 상태에서 재탈퇴 시도 시 차단 (400 DELETED_MEMBER)
...
## 4) 실패 — 이미 탈퇴한 회원이 재탈퇴 시도

탈퇴 처리 후 만료되지 않은 기존 JWT로 DELETE 재요청 시 차단된다.
...
```json
{
  ...
  "error": "DELETED_MEMBER",
  "message": "탈퇴한 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.2783995"
}
```
...
```json
{
  ...
  "error": "SUSPENDED_MEMBER",
  "message": "정지된 회원입니다.",
  "timestamp": "2026-06-26T16:59:10.6385652"
}
```
...
- [x] DELETED 회원 → 400 + `DELETED_MEMBER`(MEMBER_002)
- [x] SUSPENDED 회원 → 403 + `SUSPENDED_MEMBER`(MEMBER_003)
...
- [x] 토큰 없음 → 401 + `UNAUTHORIZED`
- [x] 이미 탈퇴한 회원 재탈퇴 → 400 + `DELETED_MEMBER`(MEMBER_002)
- [x] SUSPENDED 회원 → 403 + `SUSPENDED_MEMBER`(MEMBER_003)
