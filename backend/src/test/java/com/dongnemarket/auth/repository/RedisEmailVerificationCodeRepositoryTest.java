...
/**
 * {@link RedisEmailVerificationCodeRepository}를 실제 Redis(Testcontainers) 위에서 검증한다.
...
class RedisEmailVerificationCodeRepositoryTest {
	...
	RedisEmailVerificationCodeRepository repository;

	@BeforeEach
	void setUp() {
		repository = new RedisEmailVerificationCodeRepository(redisTemplate);
	}
	...
	void save_thenFindCode_returnsSameCode() {
	...
	void save_calledTwice_overwritesPreviousCode() {
	...
	void delete_thenFindCode_returnsEmpty() {
	...
	void getRemainingTtl_noCode_returnsEmpty() {
	...
	void getRemainingTtl_afterSave_returnsWithinConfiguredTtl() {
	...
	void save_afterTtlExpires_findCodeReturnsEmpty() throws InterruptedException {
