...
/**
 * {@code members.local_login_enabled}/{@code member_social_accounts}가 실제 MySQL(Testcontainers,
 * 운영과 동일 이미지 {@code mysql:8.0}) 위에서 Hibernate가 기대하는 타입/제약과 일치하는지 검증한다.
 * <p><b>왜 {@code ddl-auto=create}로 검증하고 V3 Flyway 스크립트로 직접 검증하지 않는가:</b>
 * V1/V2 Flyway 베이스라인이 이미 develop의 현재 엔티티(예: 매너온도 시스템의 {@code manner_ratings})보다
 * 뒤처져 있어(별도로 발견한 기존 이슈, 소셜 로그인과 무관 — PR 설명에 별도 기재), V1→V2→V3를 그대로 적용한 뒤
 * {@code ddl-auto=validate}로 전체 컨텍스트를 띄우면 이 기존 격차 때문에 {@code manner_ratings} 누락으로
 * 실패해 정작 확인하려는 소셜 로그인 컬럼 검증까지 가리지 못한다.
 * <p>대신 {@code ddl-auto=create}로 Hibernate가 현재 엔티티 그래프로부터 실제로 생성하는 DDL을 직접
 * 확인한다 — Flyway/베이스라인 상태와 무관하게, "Hibernate가 이 필드에 기대하는 컬럼 타입"이라는
 * 질문에 대한 권위 있는 답을 얻을 수 있다. {@code V3__add_social_login_support.sql}은 이 결과와
...
class V3SocialLoginSchemaValidationTest {
	...
	void hibernateGeneratedType_forLocalLoginEnabled() {
