...
public interface CommentRepository extends JpaRepository<Comment, Long> {
     ...
     * <p>{@code JOIN FETCH c.member}로 작성자를 한 쿼리에 함께 로딩해 목록 매핑 시의 N+1을 제거한다.
     * 작성자는 {@code @ManyToOne} 단건 연관이라 행 증식이 없어 컬렉션 fetch join의 메모리 페이징 문제(HHH000104)에 해당하지 않는다.
     */
    @Query("SELECT c FROM Comment c JOIN FETCH c.member " +
            "WHERE c.product.id = :productId AND c.deletedAt IS NULL " +
            "ORDER BY c.createdAt ASC")
    List<Comment> findAllWithMemberByProduct_Id(@Param("productId") Long productId);
