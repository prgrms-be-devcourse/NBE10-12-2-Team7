...
/**
 * 카카오/구글 REST API 호출 전용 {@link WebClient}. 연결/응답/쓰기 타임아웃과 응답 최대 크기를 명시적으로
 * 둔다 — 제공자가 느리거나 무한정 응답을 흘려보내도 요청 스레드가 붙잡히지 않게 하기 위함이다.
 * <p>이 WebClient에는 재시도(retry)를 붙이지 않는다 — 인가 코드 교환은 1회용 작업이라 자동 재시도가
 * 오히려 코드를 낭비하거나 의도치 않은 재사용 오류를 유발할 수 있다.
 * <p>주의: 이 애플리케이션에서 {@code reactor.netty.http.client}/
 * {@code org.springframework.web.reactive.function.client} 로거를 DEBUG 이상으로 올리지 말 것 —
 * wire 로깅에 Authorization 헤더(인가 코드 교환 시 client secret 등)와 응답 본문(access token)이 그대로
 * 찍힌다.
 */
@Configuration
public class OAuthWebClientConfig {
	...
	public WebClient oauthWebClient(
			...
			@Value("${oauth.http.connect-timeout-millis}") int connectTimeoutMillis,
			@Value("${oauth.http.response-timeout-millis}") long responseTimeoutMillis,
			@Value("${oauth.http.max-in-memory-size-bytes}") int maxInMemorySizeBytes) {
