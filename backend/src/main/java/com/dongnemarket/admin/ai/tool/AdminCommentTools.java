 ...
 */
@Component
public class AdminCommentTools {

    private final AdminCommentService adminCommentService;

    public AdminCommentTools(AdminCommentService adminCommentService) {
        this.adminCommentService = adminCommentService;
    }

    @Tool(description = "전체 댓글 목록을 조회한다. 삭제된 댓글도 포함해 반환된다. "
            + "삭제 여부 등 특정 조건이 필요하면 이 목록에서 걸러서 답하라.")
    public List<AdminCommentResponse> listComments() {
