...
public class MannerRatingController {
    ...
    public MannerRatingController(MannerRatingService mannerRatingService) {
        this.mannerRatingService = mannerRatingService;
    }

    @Operation(summary = "거래 후 별점 등록", description = "완료된 거래에 대해 구매자가 판매자에게 별점(1~5)을 남긴다.")
    @PostMapping("/api/manner/ratings")
    public ResponseEntity<ApiResponse<MannerRatingResponse>> rate(
            @AuthenticationPrincipal Long memberId,
            @Valid @RequestBody MannerRatingCreateRequest request) {
        MannerRatingResponse response = mannerRatingService.rate(memberId, request);
        return ResponseEntity.status(201)
                .body(ApiResponse.success(201, "별점이 등록되었습니다.", response));
    }
}
