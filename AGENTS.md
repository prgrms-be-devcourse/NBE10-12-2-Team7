...
| 이 작업을 왜·어떻게 했나 | [wiki/INDEX.md](wiki/INDEX.md) — PR 번호별 기록 |
...
| 새 **기술/라이브러리/구조 패턴** 도입 | 그 모듈 문서에 반영 + **[`wiki/decisions/`](wiki/decisions/)에 결정 기록** |
...
2. **작업을 마치면 [`wiki/`](wiki/INDEX.md)에 PR 번호로 기록을 남긴다.** 특히 **왜 그렇게 했는지**를 — 코드에서 복원되지 않는 유일한 정보다.
...
읽는 순서 추천: [README.md](README.md)(띄우기) → 작업할 모듈의 `<모듈>.md` → [wiki/INDEX.md](wiki/INDEX.md)(과거 작업 맥락).
